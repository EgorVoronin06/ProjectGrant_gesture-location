<script lang="ts">
    ...
    let { frame, width, height }: Props = $props();
    ...
    $effect(() => {
        ...
        // Draw pose landmarks
        if (frame.pose && frame.pose.landmarks) {
            drawPoseLandmarks(frame.pose.landmarks);
        }

        // Draw face mesh
        if (frame.face && frame.face.faceLandmarks) {
            for (const face of frame.face.faceLandmarks) {
                drawFaceMesh(face);
            }
        }
        // Draw hand landmarks
        if (frame.hands && frame.hands.landmarks) {
            for (const landmarks of frame.hands.landmarks) {
...
<div class="drawer">
    ...
    <canvas class="drawer__canvas" bind:this={outputCanvas} {width} {height}
    ></canvas>
    <div>Landmarks overlay</div>
    <div>{frame.hands?.landmarks[0]}</div>
</div>
